...
parser.add_argument("--dataroot", type=str, default="./results/local/gt")
parser.add_argument("--mask_path", type=str, default="./mask/bunny_mask_local.png")
parser.add_argument("--splitfile", type=str, default="../data/synthetic/split/test.txt")
parser.add_argument("--result_dir", type=str, default="./results/local/sold")
...
for i, item in enumerate(tqdm(items)):
    ...
    est_local_path = os.path.join(result_dir, 'relight_bunny', '%s_local_%s_bunny_relight.hdr' %(filebasename, local_id))
    gt_local_path = os.path.join(dataroot, 'relight_bunny', '%s_local_%s_bunny_relight.hdr' %(filebasename, local_id))
    gt_global = imageio.imread(gt_local_path, format='HDR-FI')  # NOTE: No scale here for relighted bunnys!
    est_global = imageio.imread(est_local_path, format='HDR-FI') # H, W, C
    gt_global_nonzero = gt_global[bunny_mask>0]
    est_global_nonzero = est_global[bunny_mask>0]
    tmp_dict = all_dict['all'].get(filebasename, {})
    _tmp_dict = {}
    if np.percentile(gt_global_nonzero, 10) > 1e-2:
        _tmp_dict['GMAE'] = float(mae(gt_global_nonzero, est_global_nonzero))
        _tmp_dict['GMSE'] = float(mse(gt_global_nonzero, est_global_nonzero))
        _tmp_dict['GRMAE'] = float(np.sqrt(_tmp_dict['GMSE']))
        gmae_list.append(_tmp_dict['GMAE'])
        gmse_list.append(_tmp_dict['GMSE'])
        grmse_list.append(_tmp_dict['GRMAE'])
    else:
        _tmp_dict['GMAE'] = -1.0
        _tmp_dict['GMSE'] = -1.0
        _tmp_dict['GRMAE'] = -1.0
    tmp_dict[local_id] = _tmp_dict
...
with open(os.path.join(output_dir, 'evaluation_local_relighting.json'), 'w') as f:
    f.write(json.dumps(all_dict, indent=2))

print("results have been saved to %s." %(os.path.join(output_dir, 'evaluation_local_relighting.json')))
