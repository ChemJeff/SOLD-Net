...
with torch.no_grad():
    ...
    for i, test_data in tqdm(enumerate(testLoader)):
        ...
        for k in range(image_recon_local.shape[0]):
            ...
            mask_est = recon_local_mask[k:k+1]
            ...
            if tb_logger and (i+1) % args.plot_every == 0:
                ...
                add_tf_summary_value(tb_logger, "MAE_GLOBAL", tmp_dict['MAE_GLOBAL'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "MSE_GLOBAL", tmp_dict['MSE_GLOBAL'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "RMSE_GLOBAL", tmp_dict['RMSE_GLOBAL'], iteration=img_idx)
                ...
                add_tf_summary_value(tb_logger, "MAE_MASK_GT", tmp_dict['MAE_MASK_GT'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "MSE_MASK_GT", tmp_dict['MSE_MASK_GT'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "MAE_MASK_EST", tmp_dict['MAE_MASK_EST'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "MSE_MASK_EST", tmp_dict['MSE_MASK_EST'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "SIL_MAE", tmp_dict['SIL_MAE'], iteration=img_idx)
                add_tf_summary_value(tb_logger, "SIL_MSE", tmp_dict['SIL_MSE'], iteration=img_idx)
...
avg_mae_global = np.array(mae_global_list, dtype=np.float64).mean()
avg_mse_global = np.array(mse_global_list, dtype=np.float64).mean()
avg_rmse_global = np.array(rmse_global_list, dtype=np.float64).mean()
...
all_dict['stat']['AVG_MAE_GLOBAL'] = avg_mae_global
all_dict['stat']['AVG_MSE_GLOBAL'] = avg_mse_global
all_dict['stat']['AVG_RMSE_GLOBAL'] = avg_rmse_global
...
print("avg_mae_global: %f, avg_mse_global: %f, avg_rmse_global: %f" %(avg_mae_global, avg_mse_global, avg_rmse_global))
